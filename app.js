...
async function login() {
  ...
  byId("userChip").textContent = currentUser.email || "Usuário logado";
  await refreshAll();
}
...
function showSection(section) {
  ...
  byId("pageTitle").textContent = titles[section][0];
  byId("pageSubtitle").textContent = titles[section][1];
}
...
async function loadDashboard() {
  ...
  byId("dashClients").textContent = clientsCount;
  byId("dashProcesses").textContent = activeProcesses;
  byId("dashDeadlines").textContent = upcoming.length;
  byId("dashPayments").textContent = moneyBR(pendingTotal);
...
function resetClientForm() {
  ...
  byId("clientPanelTitle").textContent = "Novo cliente";
  ["clientNome", "clientTelefone", "clientEmail", "clientObs"].forEach((id) => (byId(id).value = ""));
}
...
async function editClient(id) {
  ...
  editingClientId = id;
  byId("clientPanelTitle").textContent = "Editar cliente";
  byId("clientNome").value = data.nome || "";
  byId("clientTelefone").value = data.telefone || "";
  byId("clientEmail").value = data.email || "";
  byId("clientObs").value = data.observacoes || "";
...
function resetProcessForm() {
  ...
  byId("processPanelTitle").textContent = "Novo processo";
  ["processNumero", "processTipo", "processObs"].forEach((id) => (byId(id).value = ""));
  byId("processStatus").value = "ativo";
}
...
async function saveProcess() {
  ...
  const status = byId("processStatus").value;
...
async function editProcess(id) {
  ...
  editingProcessId = id;
  byId("processPanelTitle").textContent = "Editar processo";
  byId("processClient").value = data.client_id || "";
  byId("processNumero").value = data.numero || "";
  byId("processTipo").value = data.tipo || "";
  byId("processStatus").value = data.status || "ativo";
  byId("processObs").value = data.observacoes || "";
...
function resetDeadlineForm() {
  ...
  byId("deadlinePanelTitle").textContent = "Novo prazo";
  byId("deadlineDescricao").value = "";
  byId("deadlineData").value = "";
}
...
async function editDeadline(id) {
  ...
  byId("deadlinePanelTitle").textContent = "Editar prazo";
  byId("deadlineProcess").value = data.process_id || "";
  byId("deadlineDescricao").value = data.descricao || "";
  byId("deadlineData").value = data.data || "";
...
function resetPaymentForm() {
  ...
  byId("paymentPanelTitle").textContent = "Novo pagamento";
  byId("paymentValor").value = "";
  byId("paymentStatus").value = "pendente";
}
...
async function editPayment(id) {
  ...
  editingPaymentId = id;
  byId("paymentPanelTitle").textContent = "Editar pagamento";
  byId("paymentClient").value = data.client_id || "";
  byId("paymentValor").value = data.valor || "";
  byId("paymentStatus").value = data.status || "pendente";
